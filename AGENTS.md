...
- The September 10 alley clarity trial uses fixed window/transom/sign spill and restrained ground/curb/obstacle material lifts. Keep its one baked atlas and batched fixtures; no player-following fill or additional live lights/shadows. `readability=off` compares the preceding dark-pools look; `lighting=classic` also disables the trial. See [the receipt](docs/verification/alley-lighting-2026-09-10.md).
...
- Mobile uses landscape touch controls through the existing camera, muzzle and fixed physics step. Preserve analog movement, independent finger ownership, drag-to-aim firing, cancellation cleanup and concise dark HUD. No real-phone performance claim has been verified; see [mobile controls](docs/mobile-controls.md).
...
- The dated production release has server-owned **8–11 AI per round** with eleven reserved slots. Current source uses 16 total rats and automatic-room backfill; the private full-lobby fixture fills every available slot. Preserve durable bot rosters, fresh round names and human replacement. Do not add browser-hosted bots to the public room.
