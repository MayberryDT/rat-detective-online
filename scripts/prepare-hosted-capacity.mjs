...
const { values } = parseArgs({ options: { deploy:{type:'boolean',default:false}, minutes:{type:'string',default:'120'}, window:{type:'string',default:'4'}, bots:{type:'string',default:'11'}, 'checkpoint-control':{type:'boolean',default:false} } });
...
const fixture = await prepareFixture(out, { hosted:true, expiresAt:Date.now()+minutes*60000, window, serverBots, checkpointControl:values['checkpoint-control'] });
