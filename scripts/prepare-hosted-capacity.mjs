...
const { values } = parseArgs({ options: { deploy:{type:'boolean',default:false}, minutes:{type:'string',default:'120'}, window:{type:'string',default:'4'}, bots:{type:'string',default:'11'}, cap:{type:'string',default:'100'}, 'checkpoint-control':{type:'boolean',default:false} } });
...
const fixture = await prepareFixture(out, { hosted:true, expiresAt:Date.now()+minutes*60000, window, serverBots, maxPlayers:Number(values.cap), checkpointControl:values['checkpoint-control'] });
