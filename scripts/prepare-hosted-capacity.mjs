...
const { values } = parseArgs({ options: { deploy:{type:'boolean',default:false}, minutes:{type:'string',default:'120'}, window:{type:'string',default:'8'}, bots:{type:'string',default:'11'}, cap:{type:'string',default:'100'}, 'full-lobby':{type:'boolean',default:false}, 'checkpoint-control':{type:'boolean',default:false} } });
...
const fixture = await prepareFixture(out, { hosted:true, expiresAt:Date.now()+minutes*60000, window, serverBots, maxPlayers:Number(values.cap), fullLobby:values['full-lobby'], checkpointControl:values['checkpoint-control'] });
