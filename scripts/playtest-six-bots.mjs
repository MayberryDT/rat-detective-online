...
const {ChaosDecoder}=await import(pathToFileURL(`${receipt.stage}/validator.mjs`));
...
try{
 for(let i=0;i<6;i++)await new Promise((resolve,reject)=>{
  ...
  c.ws.on('open',()=>c.ws.send(JSON.stringify({type:'join',protocolVersion:5,name:names[i],appearance:{hatType:'fedora',hatColor:0x554433,furColor:0x777777,coatColor:0x333344}})));
