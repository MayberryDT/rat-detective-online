...
The three links join the same private automatic pool. Renewed backend expiry is **September 10, 2026, 12:25 PM Pacific**, private Worker `1c91db50-d6d7-44aa-856a-364beca8aa6c`. The mobile build remains unchanged from `2e94e83`; renewal receipts are under `output/mobile-preview-renewal-2026-09-10T15-25-11Z/`. See [implementation verification](verification/mobile-controls-2026-09-10.md) for the original checks. These are private preview addresses, not a production release.

Renewal checks: all 122 recorded source hashes and 45 frozen client files matched. Each route served exact HTML/JS/CSS and all 13 WAVs. Separate six-second passive sessions received eight rats, protocol 5 and zero invalid packets: desktop 157 snapshots, Wi-Fi 163, Tailscale 165. Halla independently reached the Tailscale relay. These are asset/protocol checks, not new phone gameplay or performance tests; unchanged application tests were not rerun.
