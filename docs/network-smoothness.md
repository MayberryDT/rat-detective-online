...
Production balls, cases and corpses retain the older 75 ms ChaosPresentation path. The current private preview instead uses a simulation-time cursor that advances at 90–110% playback speed toward a 100–350 ms adaptive reserve, with 32 samples per object and the existing 80 ms extrapolation limit. Delayed packets no longer shift the cursor or accumulate future catch-up debt. Gaps of at least one second, teleports and ownership changes still reset history. Ball/body ricochets now retain history and reconstruct consistent single-bounce corners, as cases already did; pending births already superseded by a collision remain cancelled. New tracks shed at most 25 ms of initial clock correction per 100 ms, preventing large reserves from freezing newly visible balls. These are rendering changes; projectile physics, lifecycle/HUD updates and immediate removals remain authoritative. See [physics playback verification](verification/physics-playback-2026-09-10.md).

The private protocol-8 shooter receives actual birth IDs, resolved velocities and simulation time, while observers keep ordinary production shot packets. The current [responsive-shooting follow-up](verification/responsive-shooting-2026-09-10.md) starts one real-ID shot immediately after successful send, in the existing instanced ball pool. Shared UUID-seeded pattern resolution makes Bad Ammunition and Scattershot agree with the authority's incident volley; membership is corrected if an incident changes in transit. The confirmation merges the existing entry and never rewinds it to the muzzle. Local swept movement and bounded replay from authoritative samples preserve immediate flight without putting the owner's shot behind the world interpolation reserve. Local prediction never causes damage, scoring, impulses or speculative hit feedback. Received removal takes precedence; snapshots predating confirmation cannot erase an unacknowledged shot. Unconfirmed entries expire after 750 ms, replay is capped at 500 ms/30 steps, and active/retired IDs stay bounded. Reset/disposal clears them.

Confirmation-only shots retain the preceding bounded first-draw muzzle correction as a fallback. The shared world playback buffer, observer packets, audio, ordinary physics and input cadence are unchanged. Production remains protocol 7. Prior implementation receipts: [muzzle delivery](verification/authoritative-muzzle-2026-09-10.md), [audio/owner-only correction](verification/audio-performance-2026-09-10.md), [shared playback](verification/physics-playback-2026-09-10.md).
