...
- Protocol **10** carries pickups and buffs in compact snapshots, including claim and expiry updates. This supersedes the briefly deployed protocol-9 candidate that omitted those fields.
...
- Validation: **883 tests** (726 client, 132 Worker, 25 script), typecheck, build and zero-vulnerability audit. Bounded browser review checked both incident modes; a separate two-client compact-v2 local Worker probe exercised real pickup claims/healing, buff deadlines and trap outcomes. Local reconnects were observed; no human feel, multi-human match, real-phone or FPS acceptance is claimed. Agent browser tests must use `&mute=1` on localhost; the public mix is unchanged.

## Accepted launcher, projectile and shooting follow-up (included in September 11 release)
...
After further playtesting Tyler reported soft audio and whole-game stutter in the same browser where production felt fine. The launcher pass had left the other audio files/gains unchanged from production, but an earlier mix had reduced the gun to .3. Current private source restores the earlier **.4 gun level** and widens the common distance-fade scale from 24 to **32** (near ceilings and source cutoffs remain). Birth payloads now go **only to the firing player**; observers retain production shot packets and snapshot playback. The pending-shot list is reused. Quiet diagnostics no longer rewrite complete history to synchronous storage or log objects during play; bounded memory/export and relay summaries remain available when explicitly enabled. **824 tests**, typecheck and both builds pass. Captured network delivery was uneven; the modest presentation CPU difference does not explain a severe frame regression. Tyler subsequently accepted the sounds and clarified that the local character/camera are smooth while balls, corpses and cases stutter. See the linked audio/performance receipt for the earlier evidence limits.

The resulting shared-physics correction replaces the short, packet-shifted playback clock with bounded 90–110% advancement toward a 100–350 ms adaptive reserve, retains 32 samples, and preserves motion history across ball/body ricochets. New balls join without freezing when the reserve grows. Captured-timing replay reduced held frames from **23.0% to 2.35%** and capped continuous-track catch-up at **1.1×** nominal speed. This is a deterministic playback result, not an FPS measurement or proof that long network gaps disappeared. Current-muzzle tests include player movement and turning during confirmation, with one ball per real ID and unchanged server trajectory. **834 tests**, typecheck and both builds pass; accepted audio is unchanged. See [physics playback receipt](verification/physics-playback-2026-09-10.md).

Tyler then reported that shared motion feels better but shooting feels delayed and intermittently invisible. Waiting for birth confirmation delayed every local ball by a round trip; a close-range ball could be removed before its first draw. The current [responsive-shooting correction](verification/responsive-shooting-2026-09-10.md) starts a single real-ID shot immediately at the animated muzzle after a successful send, in the existing instanced pool. Shared UUID-seeded volleys match Bad Ammunition/Scattershot counts and directions. Confirmation merges that entry; local swept motion is presentation only, and authoritative correction/removal wins. Indexed sweeps keep the added collision work bounded. **856 tests**, typecheck and both builds pass, including simulated 50/150/400 ms confirmation delays and real muzzle/instanced-ball tests. Tyler accepted the final shooting feel and requested these changes be committed. Audio and shared world playback are unchanged.

## Accepted fixture lighting is live

The subsequent screenshot review found visible fixture switching and misplaced facade spill. The local correction keeps streetlamp emission and pavement pools steady, excludes reassigned exterior actor spots from scenery, and prevents slightly negative grounded feet from activating sewer lights across ordinary streets. Window shafts now use the actual emissive pane UVs, occupancy, blinds and facade trim; transoms sit beneath entrance canopies. Ground spill is restricted to pavement tops. See [steady fixture lighting](verification/steady-fixture-lighting-2026-09-10.md) for checks and limits. Tyler accepted the result and requested publication. It is now committed and **deployed to production**; see [release verification](verification/steady-lighting-production-2026-09-10.md).
...
[Open target practice](http://127.0.0.1:5193/hitbox-practice.html). This separate local fixture uses four immobile rats, the actual shoulder camera/muzzle and shared hit simulation, immediate health refill after kills, and optional authoritative hitbox wires. H toggles shapes, R resets targets/counters, T returns to the firing line. No bots shoot, incidents run, or multiplayer room is joined. It includes the accepted local lighting correction above. See [practice setup and checks](hitbox-practice.md). This test environment remains separate from the live match and the existing full-game relay.

## Current production release

The accepted game changes and the pickups/Planted Evidence update are committed and live at **https://ratdetective.online/**: application commit `aaa8750`, Worker `d6d1b1e3-9406-4df6-a3f5-04132652e3c1`, protocol **10**. The canonical `public-live-v2` world remains version 2 / seed **341283204**. See [the September 11 release receipt](verification/pickups-planted-evidence-2026-09-11.md). This includes the previously accepted responsive-shooting/audio/playback follow-up.

Rooms cap at **16 total rats**. Automatic matchmaking fills occupied rooms to at least eight participants with server-owned AI, removes surplus bots as humans join, and refills vacancies after ten seconds. Empty rooms sleep; additional rooms open when needed. Version-2 rounds use Dispatch Assignments and three-second respawns. The release retains network delivery bounds, accepted incidents and case jokes, spatial audio, credits, title music and sewer approach/tunnel lighting. Title/name controls now work independently of the engine; incremental city and shader preparation plus a bounded unreserved connection move work ahead of Enter City. Mobile FIRE is **one shot per tap**, never held repeat. The latest correction keeps streetlights on at settled ground-contact height, widens their coverage, and makes fixed facade windows/doors/signs cast directional light onto rats through the same four spots. Spaced panes and stronger baked spill improve long alley walls. The shared 1.4 authored gain and rat 16% color lift/.28 emissive intensity remain; ambient and light/shadow budgets are unchanged. The scoreboard has names and YOU without AI/PLAYER badges or bot totals.

Latest release checks passed: **883 tests**, typecheck, production build and zero-vulnerability dependency audit. All **51 published files** match the final build. Health, old-host redirects, original world identity, protocol 10 and six pickup sites pass. A bounded passive observer received 50 valid compact snapshots with zero invalid messages and eight rats, then the empty room returned to zero. Browser review of both incident modes was bounded and muted after Tyler's request; local-runtime reconnects were observed. No human pickup-feel acceptance, real-phone run, controlled FPS measurement or multi-human match is claimed. [Release receipt and limits](verification/pickups-planted-evidence-2026-09-11.md).
