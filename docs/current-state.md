...
Current audio preview: [noir countdown and restrained victory](http://127.0.0.1:5186/?room=graybox-benchmark-match-deliberate-foley&diagnostics=quiet), expiring **September 10 at 3:48 AM Pacific**. Immutable client `index-fjFYSmAr.js`; private Worker remains `e39175b8-0e8d-45e2-aa64-0630ff6ff62e`. Only the audio relay on 5186 received a client refresh. All 13 clips/client bytes match the frozen fixture; 179 valid passive snapshots, eight rats, zero invalid. [Latest receipt](verification/foley-noir-2026-09-10.md).

Tyler accepted the name roll and all other retained sounds, removed **grow, charge, split, kill confirm and unstick**, and requested a darker countdown and less extravagant victory. Countdown is now a 550 ms dry clock tock with damped low body, without bells. Victory is a quieter **2.1-second muted noir-jazz phrase** with a small comic horn fall and case-closed thock. Eleven other clips and their gains are unchanged; launchers remain silent. Final name stamp still clears rolling clacks; victory retains exclusive use of the new audio bus until its phrase ends. **655 tests**, typecheck/build pass on the first run. Human listening remains for the two replacements. No merge, backend/production deployment or changes to other ongoing previews. See [current audio behavior](chaos-foley.md) and [the historical large-payoff revision](verification/foley-payoffs-2026-09-10.md).

Latest private full-game preview: [new lighting](http://127.0.0.1:5184/?room=graybox-benchmark-match-bounce-light-v12&diagnostics=quiet&lighting=pools) / [classic lighting](http://127.0.0.1:5184/?room=graybox-benchmark-match-bounce-light-v12&diagnostics=quiet&lighting=classic). Both have the new ricocheting case behavior. Private Worker `0e545734-e5c7-4f7c-bdce-ad3f603ccce4`, protocol **4**, immutable client `index-B3XezcUI.js`; expires September 10 at **2:03 AM Pacific**. Automatic rooms backfill to eight total rats. A six-second passive private-pool check received eight rats and 142 valid snapshots, zero invalid packets, and confirmed matching immutable client bytes. It did not exercise gameplay input. Receipt: `output/hosted-capacity-deployment-2026-09-10T05-03-50-102Z/deployment.json`; check: `output/tampering-lighting-2026-09-09/full-preview-check.json`; local service: `rat-detective-ricochet-lighting-preview.service`. Ports 5182 and 5183 are superseded. Public production is unchanged.
