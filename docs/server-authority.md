...
The assignment, physical chaos state, round and remaining shuffle bag share a synchronous SQLite checkpoint, normally each second and on objective/contact lifecycle changes. The final result and one reset event are written atomically before broadcasting the winner. Eviction restores the current assignment and its landmark order without rerolling or crediting elapsed offline possession. Timer/position recovery is bounded by the latest checkpoint, not a guarantee of retaining every uncheckpointed millisecond. Ordinary and compact snapshots, welcome, gameWon and gameReset carry validated assignment state. Wire protocol **4** requires a matching client and Worker; this is separate from world version 2 and the existing compact-v1/compact-v2 codec names.
