...
Private protocol 8 includes the server-created ball IDs and resolved velocities in accepted `playerShot` events, using the simulation clock and including the firing player. This supplies the actual birth before a later snapshot has already moved the ball away from its muzzle. The renderer maintains one track per actual ball, without local prediction, extra damage or a second projectile. Bad Ammunition and Scattershot use the authority's resolved volley. See [authoritative muzzle delivery](verification/authoritative-muzzle-2026-09-10.md).
