...
| City entry | Prepare the canonical public world from HTTP metadata before enabling entry; reuse it when the welcome matches. Hide the title immediately after the accepted welcome. No title socket or reserved player slot; different assigned worlds and unavailable metadata retain the normal join fallback |
