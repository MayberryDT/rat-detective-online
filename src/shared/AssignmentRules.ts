import { activeDestination, ASSIGNMENT_TUNING, destinationContains, type AssignmentState, type AssignmentResult } from './assignments';
import type { PlayerData, Vec3Data } from './networkProtocol';

export type CrossingOutcome = 'none' | 'verified' | 'closed' | 'carry-required';
...
export class AssignmentRules {
    constructor(readonly state: AssignmentState, private readonly players: Map<string, PlayerData>) {}
    ...
    visit(holder:string|null,position:Vec3Data,at:number):CrossingOutcome {
        ...
        this.state.stamps++;this.state.revision++;
        if(this.state.stamps===this.state.destinations.length){this.close(player,at,'carried');return 'closed';}
        return 'verified';
    }
}
