...
export function createCaseGrip(entity:RatEntity):THREE.Group {
        ...
        let coatMaterial:THREE.MeshStandardMaterial|undefined,skin:THREE.MeshStandardMaterial|undefined,highlight:THREE.MeshStandardMaterial|undefined;
        ...
        if(!coatMaterial||!skin||!highlight)throw new Error('Rat carry materials missing');
        ...
        const cuffStart=hand.clone().lerp(elbow,.4);
        segment(shoulder,elbow,.12,coatMaterial);segment(elbow,cuffStart,.09,coatMaterial);
        segment(cuffStart,hand,.092,highlight);
