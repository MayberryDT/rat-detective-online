...
import { OutfitStudioSubject } from './OutfitStudioSubject';
...
for(const [id,field,label,palette] of palettes){
    ...
    if(id==='highlight'){const note=document.createElement('small');note.textContent='One accent for the band, collar, lapels and cuffs.';fieldset.appendChild(note);}
...
let carried=query.get('held')!=='0',remote=query.get('presentation')==='remote'||query.has('batch');
...
function syncUi(){
    ...
    query.set('environment',environment);query.set('view',view);query.set('held',carried?'1':'0');query.set('presentation',remote?'remote':'local');query.delete('batch');
    history.replaceState(null,'',`?${query}`);
    el('outfit-label').textContent=`${el('coat-name').textContent} coat / ${el('hat-name').textContent?.toLowerCase()} hat`;
    el('case-status').textContent=carried?'Carrying the case':'Pistol only';el('render-status').textContent=remote?'Opponent':'Local rat';
    ...
    el('orbit-hint').textContent=environment==='studio'?'Drag to orbit · scroll to zoom':'Static lighting review · no gameplay inputs';
    pressed('carry',carried);pressed('idle',!walking);pressed('walk',walking);pressed('pause',paused);pressed('detail',detail);pressed('turntable',turntable);
    ...
    for(const id of ['detail','turntable','walk'])(el(id) as unknown as HTMLButtonElement).disabled=environment!=='studio';
    (el('walk') as unknown as HTMLButtonElement).title=environment==='studio'?'Preview the existing movement animation':'Walk cycle is available in the studio';
...
function rebuild(){
    ...
    subject=new OutfitStudioSubject(scene,world,camera,options,p,remote,carried);
...
async function setEnvironment(next:string){
    environment=next;walking=false;detail=false;turntable=false;controls.autoRotate=false;currentPosition.set(0,0,0);syncUi();
    if(next!=='studio'&&!city){
...
el('carry').onclick=()=>{carried=!carried;if(!preparing)subject.setCarried(carried);syncUi();};
...
renderer.setAnimationLoop(now=>{
    ...
    if(environment==='studio'){
        ...
        subject.rat.update(dt);subject.controller.updateView();city?.update(dt,camera,subject.rat.mesh.position);
    }
