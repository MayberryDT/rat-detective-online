...
const hud=document.createElement('div');hud.className='pickup-buffs';const cards=(['ironclad','hustle','quick-fix'] as const).map(kind=>{const c=powerupCard(kind);hud.appendChild(c);return c;});document.body.appendChild(hud);
...
renderer.setAnimationLoop(now=>{
    ...
    for(const [i,card] of cards.entries()){if(i===2)continue;const seconds=(i?10:12)-now/1000%(i?10:12);card.querySelector('b')!.textContent=String(Math.ceil(seconds));card.style.setProperty('--remaining',String(seconds/(i?10:12)));card.classList.toggle('powerup-expiring',seconds<3);}
