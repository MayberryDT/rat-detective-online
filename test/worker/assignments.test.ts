...
describe('shared assignment room lifecycle',()=>{
    ...
    it('restores shared stamps, mode and case identity for a late join after eviction',async()=>{
        ...
        const saved=await runInDurableObject(stub,(instance)=>{
            ...
            expect(sim.assignmentState!.stamps).toBe(1);game.persistPlayer(a,true);game.checkpointGame();return structuredClone(sim.assignmentState!);
        });
        await evictDurableObject(stub,{webSockets:'hibernate'});
        const late=await open(name,true);expect(late.welcome.round.assignment).toMatchObject({id:saved.id,roundId:saved.roundId,stamps:1,destinations:saved.destinations});
