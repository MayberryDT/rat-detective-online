...
describe('Dispatch broadcast lifecycle',()=>{
 it('shows shared progress, next destination and filing credit without double-kill instructions',()=>{
  ...
  state.assignment=createAssignment('chain-of-custody',0);state.assignment.destinations=[...CHAIN_ROUTE];state.assignment.phase='active';state.assignment.stamps=1;
  hud.update(state,4200);expect(root.querySelector('.assignment-counter').textContent).toBe('STOP 2 / 6');
  expect(root.querySelector('.assignment-progress').textContent).toBe('SEWER MAINTENANCE');
 ...
 it('confirms a verification immediately and names the next whole landmark',()=>{
  ...
  hud.update(state,5000);state.assignment.stamps=1;hud.update(state,5010);
  expect(root.querySelector('.assignment-confirmation').textContent).toContain('STAMPED! 1/6');
  expect(root.querySelector('.assignment-progress').textContent).toBe('SEWER MAINTENANCE');expect(feedback).toHaveBeenCalledWith('verified');hud.dispose();
 });
 ...
 it('announces ownership changes once, retaining a quiet readable status afterward',()=>{
  ...
  expect(root.querySelector('.case-ledger small').textContent).toContain('KEEP THE CASE CLOSE');
 ...
 it('announces the local carrier without obsolete multiplier instructions',()=>{
  ...
  expect(root.querySelector('.case-broadcast span').textContent).toBe('KEEP IT CLOSE');
  hud.update(state,5000,'Constable Trap',true);
  expect(root.querySelector('.case-ledger small').textContent).toContain('KEEP THE CASE CLOSE');hud.dispose();
 });
 it('distinguishes your pickup/loss from others and emits one cue per ownership transition',()=>{
  ...
  expect(root.querySelector('.case-broadcast span').textContent).toContain('GET IT BACK');
