...
describe('assignment navigation in the actual city',()=>{
    it.each(CHAIN_ROUTE)('a server bot enters the whole %s landmark using existing geometry',id=>{
        ...
        const completed=CHAIN_ROUTE.indexOf(id);assignment.stamps=completed;assignment.liveAt=now;assignment.phase='active';sim.setAssignment(assignment);
        ...
        try{
            for(let frame=1;frame<=1200&&sim.assignmentState!.stamps===completed;frame++){
                const at=now+frame*1000/60;
                controller.step(1/60,at,players,sim.snapshot(false),true);sim.step(1/60,at);
            }
            expect({stamps:sim.assignmentState!.stamps,position:{x:bot.x,y:bot.y,z:bot.z}}).toMatchObject({stamps:completed+1});
            if(completed===5)expect(sim.assignmentState!.result).toMatchObject({winnerId:bot.id,method:'carried'});
