...
describe('nearby Dispatch readiness siren',()=>{
 it('plays one nearby ready cue, reuses its buffer and bounds periodic repetition',()=>{
  ...
  expect(nodes).toHaveLength(1);nodes[0].onended();ctx.currentTime=6.9;audio.update(true,5);expect(nodes).toHaveLength(1);
  ctx.currentTime=7;audio.update(true,5);expect(nodes).toHaveLength(2);expect(nodes[1].buffer).toBe(nodes[0].buffer);
 ...
 it('is silent while unavailable, out of range or autoplay-blocked, and never resumes the context',()=>{
  const {audio,ctx,nodes}=fixture();audio.update(false,0);audio.update(true,40);audio.update(true,Infinity);audio.update(true,NaN);
 ...
 it('fades with distance, including a zero-distance ceiling',()=>{
  const {audio,ctx,nodes,gains}=fixture();audio.update(true,0);expect(gains[0].gain.value).toBe(.12);
  nodes[0].onended();ctx.currentTime=7;audio.update(true,20);expect(gains[1].gain.value).toBe(.03);audio.dispose();
 });
 it('cuts off when triggered or when leaving, then announces a fresh ready cycle',()=>{
  ...
  audio.update(true,0);expect(nodes).toHaveLength(2);audio.update(true,45);expect(nodes[1].stop).toHaveBeenCalledTimes(1);audio.dispose();
 });
