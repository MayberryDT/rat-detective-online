...
describe('assignments in the real city case simulation',()=>{
    it('persists authoritative case-time totals across theft and restore, and clears them on reset',()=>{
        const {sim,a,b}=fixture('excessive-force');pickup(sim,a,-16,-30);frames(sim,60);
        expect(sim.snapshot(false).possession.a).toBeCloseTo(1,6);
        sim.release(a.id);Object.assign(a,{x:100,y:.3,z:-100});
        frames(sim,30,NOW+1000);expect(sim.snapshot(false).possession.a).toBeCloseTo(1,6);
        pickup(sim,b,-16,-30,NOW+1500);frames(sim,60,NOW+1500);
        const saved=sim.snapshot(false);expect(saved.possession.b).toBeCloseTo(1,6);
        const restored=fixture('excessive-force',saved);
    ...
    it('does not score loose cases or turn an uncarried arrival into a third delivery',()=>{
        ...
        expect(sim.assignmentState!.deliverySerial).toBe(0);
        sim.assignmentState!.deliverySerial=5;sim.caseBody.position.set(-16,1.3,-34);sim.caseBody.velocity.set(0,0,-30);frames(sim,12,NOW+1000);
...
it.each(CHAIN_ROUTE)('respawns delivered %s evidence at a clear authored site and never banks a second point automatically',id=>{
    const {sim,a,b}=fixture('chain-of-custody');
    sim.assignmentState!.destinations=[id,...CHAIN_ROUTE.filter(d=>d!==id)];
    pickup(sim,a,-16,-30);carriedEntry(sim,a,id,NOW+100);
    const first=sim.snapshot(false),site=first.case.p;
    expect(first.assignment!.deliveries).toEqual({a:1});expect(first.case.owner).toBeNull();
    expect(CASE_SPAWNS.some(p=>Math.hypot(p.x-site.x,p.y-site.y,p.z-site.z)<.001)).toBe(true);
    expect(Math.hypot(site.x-a.x,site.z-a.z)).toBeGreaterThanOrEqual(30);
    expect(CHAIN_ROUTE.some(d=>destinationContains(d,site))).toBe(false);
    expect(first.case.v).toEqual({x:0,y:0,z:0});expect(first.case.missileOwner).toBeUndefined();expect(first.case.returningUntil).toBe(0);
    frames(sim,30,NOW+101);expect(sim.assignmentState!.deliveries).toEqual({a:1});expect(sim.assignmentState!.result).toBeUndefined();
    // The new position and credit survive a restore; a different rat can collect
    // the physically settled case without any teleported test pickup.
    const saved=sim.snapshot(false),restored=fixture('chain-of-custody',saved);
    Object.assign(restored.b,{x:saved.case.p.x,y:saved.case.p.y-.8,z:saved.case.p.z});restored.sim.step(0,NOW+800);
    expect(restored.sim.caseHolderId).toBe(b.id);expect(restored.sim.assignmentState!.deliveries).toEqual({a:1});
